...
def fetch_kraken_ohlc(pair: str, interval_mins: int = 1440) -> pd.DataFrame:
    ...
    try:
        ...
        # Extra guard (older pandas envs)
        if hasattr(df["time"].dt, "tz") and df["time"].dt.tz is not None:
            df["time"] = df["time"].dt.tz_localize(None)
        return df
    except Exception as e:
        out = pd.DataFrame(columns=["time","open","high","low","close","vwap","volume","count"])
...
def run_xrp_tab():
    st.header("XRP — Risk & Regimes (Kraken) + Next-Day HIGH Prediction")
    st.caption("Kraken daily OHLC + risk analytics. Prediction shows only the next-day HIGH.")
    ...
    with colB:
        with st.popover("Advanced: derive features from last Kraken row"):
            ...
            features = {
                ...
                "marketCap": float(0.0),  # optionally fetch CG market cap to refine
